...
public class TransportHelper
{
	...
	public static ProcessResult ProcessResponse(ConnectionBase connection, JSONObject response)
	{
		...
		if (connection == null)
		{
			Log.e(TAG, "TransportHelper.ProcessResponse - connection is null");
			return result;
		}
		...
		if (messagesArray != null)
		{
			...
			for (int i = 0; i < messagesArray.length(); i++)
			{
				...
				{
					connection.OnLog(LogLevel.Error, "Error when calling setMessage on connection. %s", e);
				}
			}
	...
	public static String GetReceiveQueryString(ConnectionBase connection, String data, String transport)
	{
		...
		{
			connection.OnLog(LogLevel.Error, "Unsupported message encoding error, when encoding connectionToken.");
		}
		if (connection.getMessageId() != null)
		{
			...
			{
				connection.OnLog(LogLevel.Error, "Unsupported message encoding error, when encoding messageid.");
			}
		}
		...
		if (connection.getGroupsToken() != null && connection.getGroupsToken().length() > 0)
		{
			...
			{
				connection.OnLog(LogLevel.Error, "Unsupported message encoding error, when encoding groupsToken.");
			}
		}
		...
		if (data != null)
		{
			...
			{
				connection.OnLog(LogLevel.Error, "Unsupported message encoding error, when encoding connectionData.");
			}
		}
		...
		if (connection.getQueryString() != null && connection.getQueryString().length() > 0)
		{
			...
			{
				connection.OnLog(LogLevel.Error, "Unsupported message encoding error, when encoding queryString.");
			}
		}
